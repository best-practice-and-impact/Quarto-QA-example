...
class LinRegression():
    ...
    def calculate_residuals_array(self, x, y):
        ...
        Returns
        -------
            residuals : Residuals, i.e. the difference between predicted and 
    ...
    def gradient_descent(self, x, y):
        ...
        learning_rate : a tuning parameter dictating the step size taken when 
        adjusting the parameters of the model at each iteration (float)
    ...
    def fit_model(self, x, y):
        ...
        Parameters
        -------
        theta_0 : y-intercept of the model (float, defaults to 0)

        theta_1 : gradient of the model (float, defaults to 0)

        x : Training data (array-like of floats)

        y : The target/actual y-values at each point x (array-like of floats)

        learning_rate : Model tuning parameter dictating the step size taken when 
        adjusting the parameters of the model at each iteration (float)

        convergence threshold : Threshold for considering the model fully trained 
        (float)
        """
